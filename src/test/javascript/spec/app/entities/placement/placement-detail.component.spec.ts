/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { Observable } from 'rxjs/Observable';

import { MybizdevTestModule } from '../../../test.module';
import { PlacementDetailComponent } from '../../../../../../main/webapp/app/entities/placement/placement-detail.component';
import { PlacementService } from '../../../../../../main/webapp/app/entities/placement/placement.service';
import { Placement } from '../../../../../../main/webapp/app/entities/placement/placement.model';

describe('Component Tests', () => {

    describe('Placement Management Detail Component', () => {
        let comp: PlacementDetailComponent;
        let fixture: ComponentFixture<PlacementDetailComponent>;
        let service: PlacementService;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [MybizdevTestModule],
                declarations: [PlacementDetailComponent],
                providers: [
                    PlacementService
                ]
            })
            .overrideTemplate(PlacementDetailComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(PlacementDetailComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(PlacementService);
        });

        describe('OnInit', () => {
            it('Should call load all on init', () => {
                // GIVEN

                spyOn(service, 'find').and.returnValue(Observable.of(new HttpResponse({
                    body: new Placement(123)
                })));

                // WHEN
                comp.ngOnInit();

                // THEN
                expect(service.find).toHaveBeenCalledWith(123);
                expect(comp.placement).toEqual(jasmine.objectContaining({id: 123}));
            });
        });
    });

});
